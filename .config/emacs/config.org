...
*** Poetry
#+begin_src elisp
;; FIXME: Transient complains about outdated seq version when installing poetry
...
(use-package poetry
  :elpaca t
  :after python pyvenv
  :hook (pyvenv-mode . poetry-tracking-mode)
  :preface
  ;; https://github.com/cybniv/poetry.el/issues/39#issuecomment-1259055166
  (defun yn/poetry-reload-venv ()
    "Manually update the Poetry virtual environment"
    (interactive)
    (pyvenv-deactivate)
    ...
    (setq poetry-project-venv nil
          ...
          poetry-saved-venv nil)
    ;; Re-deduce our project directory and venv, and activate it.
    (poetry-venv-workon)
    (yn/eglot-reconnect)) ;; Reload language server to use the new venv.
  :custom
  ...
  (:keymaps 'python-mode-map
   :states '(normal emacs)
   :prefix "SPC"
   "p u" '(yn/poetry-reload-venv :wk "Reload Poetry virtual environment")))
#+end_src
